...
<style>
  ...
  .score-num { font-size:28px; font-weight:600; line-height:1; }
  .score-lbl { font-size:8px; letter-spacing:0.8px; margin-top:2px; opacity:0.8; font-family:monospace; }
  .card-body { flex:1; padding:16px 18px; }
  .domain-pill { font-size:10px; font-weight:600; letter-spacing:1px; padding:2px 9px; border-radius:3px; display:inline-block; margin-right:6px; font-family:monospace; }
  .topic-pill { font-size:10px; color:#64748b; background:#f1f5f9; padding:2px 9px; border-radius:3px; display:inline-block; margin-right:6px; font-family:monospace; }
  .headline { font-size:17px; font-weight:600; color:#0f172a; line-height:1.45; margin:8px 0; }
  .summary { font-size:14px; color:#475569; line-height:1.75; margin-bottom:10px; }
  .signal-box { font-size:13px; color:#64748b; font-style:italic; padding-left:10px; margin-bottom:8px; }
  .signal-label { font-family:monospace; font-size:9px; font-weight:600; font-style:normal; letter-spacing:0.5px; }
  .context-banner { background:#fef9c3; border-left:3px solid #eab308; border-radius:3px; padding:8px 12px; margin-bottom:8px; font-size:12px; color:#854d0e; }
  .social-section { border-top:1px solid #f1f5f9; padding-top:10px; margin-top:10px; }
  .social-header { font-size:9px; color:#94a3b8; letter-spacing:0.5px; margin-bottom:6px; font-family:monospace; }
  .social-item { background:#f8fafc; border-radius:4px; padding:8px 12px; margin-bottom:5px; border-left:2px solid #d97706; font-size:13px; color:#374151; line-height:1.6; }
  .social-handle { font-size:10px; font-weight:600; font-family:monospace; }
  .unverified-badge { font-size:9px; background:#fef2f2; color:#dc2626; padding:1px 5px; border-radius:2px; font-weight:600; margin-left:6px; font-family:monospace; }
</style>
...
def main():
    ...
    starred_ids = set()  # No longer used but kept for render_card compatibility

    tab_summary, tab_news, tab_social, tab_local, tab_archive = st.tabs([
        "📊 Summary", "📰 News", "📱 Social", "🏙 Local", "🗄 Archives"
    ])
    ...
    with tab_news:
        ...
        for idx, dig in enumerate(digests):
            ...
            with st.expander(f"📅 {ts}  ·  {len(news_only)} stories  ·  {verified} verified  ·  avg {avg_d}", expanded=(idx==0)):
                for s in news_only:
                    render_card(s, starred_ids, digest_ts=ts, key_prefix=f"news_{idx}")
    ...
    with tab_social:
        ...
        for idx, dig in enumerate(digests):
            ...
            if not soc_st:
                continue
            with st.expander(f"📱 {ts}  ·  {len(soc_st)} social stories", expanded=(idx==0)):
                ...
                for s in soc_st:
                    ...
                    if any(sig.get("platform") in sel for sig in sigs) or (not sigs and s.get("standalone_social")):
                        render_card(s, starred_ids, digest_ts=ts, key_prefix=f"soc_{idx}")

    # ── LOCAL ────────────────────────────────────────────────────
    LOCAL_KEYWORDS = {
        "maharashtra", "mumbai", "thane", "pune", "nagpur", "nashik",
        "navi mumbai", "bmc", "mmc", "mhada", "mmrda", "best bus",
        "konkan", "vidarbha", "marathwada", "aurangabad",
    }
    LOCAL_SOURCES = {"TOI Mumbai", "Mid-Day Mumbai", "HT Mumbai", "Free Press Journal", "Maharashtra Times"}
    ...
    with tab_local:
        if not digests:
            ...
            for idx, dig in enumerate(digests):
                ...
                # Filter: source is a local outlet OR headline/summary contains local keyword
                local_stories = []
                for s in d_st:
                    srcs = set(s.get("sources", []))
                    text = (s.get("headline","") + " " + s.get("summary","") + " " + s.get("topic","")).lower()
                    is_local_src = bool(srcs & LOCAL_SOURCES)
                    is_local_kw  = any(kw in text for kw in LOCAL_KEYWORDS)
                    if is_local_src or is_local_kw:
                        local_stories.append(s)

                if not local_stories:
                    continue

                with st.expander(f"🏙 {ts}  ·  {len(local_stories)} local stories", expanded=(idx==0)):
                    for s in local_stories:
                        render_card(s, set(), digest_ts=ts, key_prefix=f"local_{idx}")
    ...
    with tab_archive:
        ...
        for idx, dig in enumerate(archives):
            ...
            with st.expander(f"🗄 {ts}  ·  {len(d_st)} stories  ·  avg {avg_d}", expanded=False):
                for s in d_st:
                    render_card(s, set(), digest_ts=ts, key_prefix=f"arch_{idx}")
