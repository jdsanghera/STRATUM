...
GITHUB_REPO    = "STRATUM"
GITHUB_BRANCH  = "main"
GITHUB_TOKEN   = ""   # Add your GitHub PAT here (read:repo scope)
                       # Or add as Streamlit Secret: st.secrets["GITHUB_TOKEN"]
...
def clean(text):
    """Strip HTML tags and escape special characters for safe HTML insertion."""
    if not text:
        return ""
    # Strip tags first
...
def render_card(story, starred_ids, digest_ts=None):
    ...
    topic   = clean(story.get("topic", ""))
    sources = " · ".join(clean(s) for s in story.get("sources", []))
    headline = clean(story.get("headline", ""))
    summary  = clean(story.get("summary", ""))
    signal   = clean(story.get("signal", ""))
    ...
    if sigs:
        ...
        for sig in sigs[:3]:
            ...
            sig_items += f"""<div class="social-item">
              ...
              {link}
              <div style="margin-top:4px;">{clean(sig.get('text',''))[:180]}</div>
            </div>"""
        ...
        if st.button(star_label, key=f"star_{sid}", use_container_width=True):
            if is_star:
                ...
                starred.append({
                    ...
                    "expires_at": (datetime.now(timezone.utc).replace(day=datetime.now().day + 90)).isoformat(),
                    "story":      story,
                })
