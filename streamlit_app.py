...
def render_detail_panel(story, tab_key=""):
    sc, sbg, label = score_color(story["score"])
    dc       = DOMAIN_COLORS.get(story.get("domain",""), "#475569")
    trend    = story.get("trend","stable")
    ts       = TREND_SYMBOLS.get(trend,"─")
    ...
    if st.button("✕ Close", key=f"close_detail_{tab_key}"):
...
def main():
    ...
    tab_summary, tab_global, tab_india, tab_finance, tab_social, tab_chatter, tab_local, tab_archive = st.tabs([
        "📊 Summary", "🌍 Global", "🇮🇳 India", "💹 Finance", "📱 Social", "💬 Chatter", "🏙 Local", "🗄 Archives"
    ])
    ...
    def maybe_show_detail(tab=""):
        sel = st.session_state.get("selected_story")
        if sel:
            render_detail_panel(sel, tab_key=tab)
    ...
    INDIA_SOURCES = {
        "Times of India","The Hindu","Indian Express","NDTV News","LiveMint Markets",
        "LiveMint Companies","Moneycontrol","NDTV Profit","Hindu Business","CNBC TV18",
        "ET Top Stories","Business Standard","Financial Express","ET Markets","Mint Economy",
        "Business Line","MC Markets","India Today Biz","Reuters India","BQ Prime",
        "Moneycontrol News","ET Markets TG","CNBC TV18 TG","NSE India Official","BSE India",
        "SEBI Updates","Nifty Trader","StockEdge","Trendlyne","The Hindu TG",
        "Indian Express TG","Wire News","PTI News","ANI News","NDTV TG","Scroll.in",
        "AP News","Reuters","PTI_News","CNBCTV18Live","ETNOWlive","AndyMukherjee72",
        "BSEIndia","NSEIndia","RBI","FinMinIndia","PIBIndia","bsindia","livemint",
        "ETPrimeNews","Reuters_India",
    }
    INDIA_KEYWORDS = {
        "india","indian","nse","bse","sensex","nifty","rbi","sebi","rupee","inr",
        "mumbai","delhi","bangalore","bengaluru","hyderabad","chennai","kolkata",
        "modi","government of india","ministry","budget","gst","inflation india",
        "ipo","fii","dii","nifty50","bank nifty","reliance","tata","infosys","wipro",
        "hdfc","icici","sbi","bajaj","adani","mahindra","hindustan","maruti",
    }
    FINANCE_DOMAINS = {"markets","macro","commodities","corporate","finance"}
    GLOBAL_REGIONS  = {"global","europe","middleeast","japan","asiapac"}

    def is_india_story(s):
        sources = set(s.get("sources",[]))
        text    = (s.get("headline","")+" "+s.get("summary","")+" "+s.get("topic","")).lower()
        return bool(sources & INDIA_SOURCES) or any(kw in text for kw in INDIA_KEYWORDS) or s.get("domain","") == "india"

    def is_global_story(s):
        return not is_india_story(s) and not s.get("standalone_social")

    def is_finance_story(s):
        return s.get("domain","") in FINANCE_DOMAINS

    # ── GLOBAL ───────────────────────────────────────────────────
    with tab_global:
        maybe_show_detail("global")
        if not digests:
        ...
        for idx, dig in enumerate(digests):
            ...
            items = sorted(
                [s for s in d_st if is_global_story(s) and s.get("score",0) >= MIN_SCORE],
                key=lambda x: x.get("score",0), reverse=True
            )
            verified = sum(1 for s in items if s.get("score",0)>=80)
            avg_d    = round(sum(s.get("score",0) for s in items)/len(items)) if items else 0
            with st.expander(f"🌍 {ts}  ·  {len(items)} stories  ·  {verified} verified  ·  avg {avg_d}", expanded=(idx==0)):
                render_card_grid(items, f"global_{idx}")

    # ── INDIA ────────────────────────────────────────────────────
    with tab_india:
        maybe_show_detail("india")
        if not digests:
            st.info("No digests yet."); return
        for idx, dig in enumerate(digests):
            ts    = dig.get("timestamp_ist", dig.get("timestamp",""))
            d_st  = dig.get("stories",[])
            items = sorted(
                [s for s in d_st if is_india_story(s) and not s.get("standalone_social") and s.get("score",0) >= MIN_SCORE],
                key=lambda x: x.get("score",0), reverse=True
            )
            verified = sum(1 for s in items if s.get("score",0)>=80)
            avg_d    = round(sum(s.get("score",0) for s in items)/len(items)) if items else 0
            with st.expander(f"🇮🇳 {ts}  ·  {len(items)} stories  ·  {verified} verified  ·  avg {avg_d}", expanded=(idx==0)):
                render_card_grid(items, f"india_{idx}")

    # ── FINANCE ──────────────────────────────────────────────────
    with tab_finance:
        maybe_show_detail("finance")
        if not digests:
            st.info("No digests yet."); return
        for idx, dig in enumerate(digests):
            ts    = dig.get("timestamp_ist", dig.get("timestamp",""))
            d_st  = dig.get("stories",[])
            items = sorted(
                [s for s in d_st if is_finance_story(s) and not s.get("standalone_social") and s.get("score",0) >= MIN_SCORE],
                key=lambda x: x.get("score",0), reverse=True
            )
            verified = sum(1 for s in items if s.get("score",0)>=80)
            avg_d    = round(sum(s.get("score",0) for s in items)/len(items)) if items else 0
            with st.expander(f"💹 {ts}  ·  {len(items)} stories  ·  {verified} verified  ·  avg {avg_d}", expanded=(idx==0)):
                render_card_grid(items, f"fin_{idx}")

    # ── SOCIAL ───────────────────────────────────────────────────
    with tab_social:
        maybe_show_detail("social")
        if not digests:
            st.info("No digests yet."); return
    ...
    with tab_chatter:
        ...
        for idx, dig in enumerate(digests):
            ts    = dig.get("timestamp_ist", dig.get("timestamp",""))
            d_st  = dig.get("stories",[])
            # Chatter = standalone social (any score) OR score < 50
            chatter = sorted(
                [s for s in d_st if s.get("standalone_social") or s.get("score", 0) < 50],
                key=lambda x: x.get("score", 0), reverse=True
            )
            if not chatter:
                continue
