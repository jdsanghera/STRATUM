...
def main():
    ...
    def is_social_story(s):
        """Story belongs in Social if it's standalone social OR has social signals."""
        return s.get("standalone_social", False) or bool(s.get("social_signals", []))

    def is_news_story(s):
        """Story belongs in News if it has wire/publication sources (not purely social)."""
        sources = s.get("sources", [])
        has_wire = any(
            not src.startswith("@") and not src.startswith("TG:") and not src.startswith("bsky:")
            for src in sources
        )
        return has_wire or (not sources and not s.get("standalone_social", False))
    ...
    with tab_news:
        ...
        for idx, dig in enumerate(digests):
            ...
            news_only = [s for s in d_st if is_news_story(s) and not s.get("standalone_social")]
    ...
    with tab_social:
        ...
        for idx, dig in enumerate(digests):
            ts     = dig.get("timestamp_ist", dig.get("timestamp",""))
            d_st   = dig.get("stories",[])
            # Show: standalone social posts + stories with social signals + breaking on social
            soc_st = [s for s in d_st if
                      s.get("standalone_social") or
                      s.get("breaking_on_social") or
                      s.get("social_signals")]
            if not soc_st:
                continue
            with st.expander(f"📱 {ts}  ·  {len(soc_st)} stories with social signals", expanded=(idx==0)):
    ...
    with tab_local:
        ...
        for idx, dig in enumerate(digests):
            ...
            # Also check clustered stories for local keywords as fallback
            d_st = dig.get("stories",[])
            local_stories = []
            for s in d_st:
            ...
            with st.expander(f"🏙 {ts}  ·  {len(local_items)} raw items  ·  {len(local_stories)} clustered stories", expanded=(idx==0)):
                if local_stories:
                ...
                if local_items:
                    ...
                    for item in local_items[:30]:
                        src    = safe(item.get("source",""))
                        title  = safe(item.get("title",""))
                        summ   = safe(item.get("summary",""))
                        url    = item.get("url","")
                        ...
                        st.html(f"""
                        <div style="background:#fff;border-radius:6px;padding:12px 16px;margin-bottom:8px;border:1px solid #e2e8f0;border-left:3px solid #be185d;">
                          <div style="font-size:10px;color:#94a3b8;font-family:monospace;margin-bottom:4px;">{src}</div>
                          <div style="font-size:14px;font-weight:600;color:#0f172a;line-height:1.4;margin-bottom:6px;">{title}</div>
                          <div style="font-size:12px;color:#475569;line-height:1.6;margin-bottom:8px;">{summ}</div>
                          {link}
                        </div>""")
