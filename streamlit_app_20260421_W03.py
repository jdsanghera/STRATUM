...
def main():
    ...
    with tab_news:
        ...
        for idx, dig in enumerate(digests):
            ...
            # All wire/publication stories with score >= 25, sorted by score
            news_only = sorted(
                [s for s in d_st if is_news_story(s) and s.get("score",0) >= MIN_SCORE],
    ...
    with tab_social:
        ...
        for idx, dig in enumerate(digests):
            ts     = dig.get("timestamp_ist", dig.get("timestamp",""))
            d_st   = dig.get("stories",[])
            # All stories with social signals or standalone social, score >= 25
            soc_st = sorted(
                [s for s in d_st if is_social_story(s) and s.get("score",0) >= MIN_SCORE],
                key=lambda x: x.get("score",0), reverse=True
            )
            if not soc_st:
                continue
    ...
    with tab_local:
        ...
        for idx, dig in enumerate(digests):
            ...
            for s in d_st:
                ...
                if any(kw in text for kw in LOCAL_KEYWORDS) and s.get("score",0) >= MIN_SCORE:
